...
public class Axes 
{
	...
	private EntityS _xAxis;
	private EntityS _yAxis;
	private EntityS _zAxis;
	public Axes(Loader loader)
	{
		...
		_xAxis = new EntityS(_xModel, zero, new Vector3f(0f, 0f, 0f), size);
		_xAxis.setColor(new Vector3f(1f, 0f, 0f));
	
		_yAxis = new EntityS(_yModel, zero, new Vector3f(0f, 0f, 0f), size);
		_yAxis.setColor(new Vector3f(0f, 1f, 0f));
		
		_zAxis = new EntityS(_zModel, zero, new Vector3f(0f, 0f, 0f), size);
		_zAxis.setColor(new Vector3f(0f, 0f, 1f));
	}
	
	public EntityS[] getEntities()
	{
		EntityS[] bebra = {_xAxis, _yAxis, _zAxis};
		//Entity[] bebra = {_xAxis};
