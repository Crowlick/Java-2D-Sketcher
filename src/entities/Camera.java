...
public class Camera 
{
	private Vector3f _pos = new Vector3f(0f, 0f, 1f);
	...
	private Vector3f _mouseScreenPos = new Vector3f();
	private float _camSpeed = 0.01f;
	private float _camSens = 0.02f;
	

	public Camera() {}
	
	public void move(ArrayList<Renderable> sceneComps)
	{		
		int wheel = Mouse.getDWheel();
		
		if (wheel < 0)
		{
			for (Renderable l : sceneComps)
				l.scale(1f / 1.01f);
			_pos.z /= 1.01f;
		}
		if (wheel > 0)
	...
	public Vector3f getMouseScreenPos() {return _mouseScreenPos;}
